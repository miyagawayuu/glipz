...
func RunBookmarks(ctx context.Context, pool *pgxpool.Pool) error {
	...
	hasFederationIncoming := 0
	if err := pool.QueryRow(ctx, `
		SELECT COUNT(*)::int FROM information_schema.tables
		WHERE table_schema = 'public' AND table_name = 'federation_incoming_posts'
	`).Scan(&hasFederationIncoming); err != nil {
		return fmt.Errorf("migrate bookmarks: check federation incoming: %w", err)
	}
