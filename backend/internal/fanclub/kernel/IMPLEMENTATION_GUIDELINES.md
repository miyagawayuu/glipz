...
The Patreon reference implementation is in `internal/fanclub/patreon/`.
...
2. **Create `internal/fanclub/<id>/`** with:
   - Configuration struct loaded from env (see `internal/config` for Patreon’s `PATREON_*` pattern).
...
5. **Note paywall**: in `notePremiumProjection` (or equivalent), branch on the note’s (and author’s) paywall fields. For Patreon, see `viewerEntitledToAuthorPatreonTier` in `internal/httpserver/patreon_handlers.go`. A new provider adds its own `viewerEntitled…` (or a single dispatcher reading `paywall_spec`).
