...
Membership entitlement の発行は意図的に制限されています。現在の Glipz 実装では、リモートインスタンスが Patreon または Gumroad locked posts 用の `entitlement_jwt` を発行することはできません。閲覧者は origin instance 上で membership unlock を行うか、利用できる場合は password-based unlock を使ってください。
...
- Patreon と Gumroad lock に対する remote membership entitlement minting はサポートされていません。
