...
Membership entitlement の発行は意図的に制限されています。Origin 側の `POST /federation/posts/{postID}/entitlement` は、origin がリモート閲覧者の外部会員状態を安全に検証できない provider（Patreon と Gumroad を含む）に対して `federation_membership_entitlement_unsupported` を返します。
...
Gumroad locked federated post は現時点では origin-instance only です。Viewer instance は remote origin に対する license ownership 関係を検証できないため、閲覧者は origin instance を開いて Gumroad license flow を使うか、利用できる場合は password-based unlock を使ってください。
...
- Patreon と Gumroad lock に対する origin-side remote membership entitlement minting はサポートされていません。Patreon の cross-instance unlock は上記の viewer-instance verification path でサポートされますが、Gumroad は origin-instance only です。
